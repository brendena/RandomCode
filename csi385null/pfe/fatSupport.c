// Team null
// Date: 01/28/2015
// Provided fatSupport.c file (from package.tgz)
...
{
  int bytes_read;
  if (fseek(FILE_SYSTEM_ID, (long) sector_number * (long) BYTES_PER_SECTOR,
            SEEK_SET) != 0)
  {
...
void set_fat_entry(int fat_entry_number, int value, unsigned char* fat)
{
  ...
  // Two FAT12 entries are stored into three bytes;
  // if these bytes are uv,wx,yz then the two FAT12 entries are xuv and yzw
  // Let 0a,bc denote the fat_entry_number, written as two bytes (high and low,
  // respectively)
